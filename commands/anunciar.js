...
module.exports = {
    ...
    if(!message.member.roles.some(r=>["652592408550178827", "654667200124289034"].includes(r.id)))
        ...
        if (match === null) {
            message.channel.send("<a:sino:654177101322256395> Você precisa adicionar uma <:image:653462486544023552>`IMAGEM` valida que tenha o final `.jpg .gif .png`")
            return
        }

        if (!canalMencionado) {
         message.channel.send("<:stop:653479507650674729> Você precisa mencionar sua loja ex `#loja-victor`")
            return
        }
